...
REMOTE_JN_SCRIPTS_PATH = os.path.join(REMOTE_SCRIPTS_PATH, "blenano")
REMOTE_TMP_PATH = "/home/user/tmp"
REMOTE_FIRMWARE_PATH = os.path.join(REMOTE_TMP_PATH, "firmware.blenano.hex")
...
if __name__=="__main__":
  ...
  if os.path.isdir(job_dir):
   for f in os.listdir(job_dir):
    if f.endswith(".blenano.hex"):
      firmware_path = os.path.join(job_dir, f)
      break
       
  if firmware_path == None:
    print "No blenano firmware found!"
    sys.exit(2)
      
  hosts_path = os.path.join(job_dir, "hosts")
  # Copy firmware to the nodes
  if pscp(hosts_path, firmware_path, REMOTE_FIRMWARE_PATH, "Copying blenano firmware to the PI nodes") != 0:
    sys.exit(3)
  # Program the nodes
  if pssh(hosts_path, "%s %s"%(os.path.join(REMOTE_JN_SCRIPTS_PATH, "install.sh"), REMOTE_FIRMWARE_PATH), "Installing blenano firmware") != 0:
    sys.exit(4)
  # Start serialdump
